        ...
        <Modal isOpen={true} onClose={onClose} title="Nueva Solicitud de Servicio">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-4">
                    ...
                    <Button type="submit" disabled={isSubmitting}>{isSubmitting ? "Guardando..." : "Guardar Solicitud"}</Button>
                </div>
            </form>
        </Modal>
    );
...
const WorkDoneFormModal: React.FC<WorkDoneFormModalProps> = ({ workOrder, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);
        await updateWorkOrderWorkDone(workOrder.id, completionDate, materialsUsed);
        setIsSubmitting(false);
        onClose();
    };
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title="Registrar Trabajo Realizado">
             <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-4">
                    <Button type="button" variant="secondary" onClick={onClose} disabled={isSubmitting}>Cancelar</Button>
                    <Button type="submit" disabled={isSubmitting}>{isSubmitting ? "Guardando..." : "Guardar Trabajo"}</Button>
                </div>
             </form>
        </Modal>
    );
...
const ApprovalFormModal: React.FC<ApprovalFormModalProps> = ({ workOrder, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSubmitting(true);
        await updateWorkOrderApproval(workOrder.id, approvalName, approvalDate);
        setIsSubmitting(false);
        onClose();
    };
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title="Registrar Conformidad del Solicitante">
             <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-4">
                    <Button type="button" variant="secondary" onClick={onClose} disabled={isSubmitting}>Cancelar</Button>
                    <Button type="submit" variant="success" disabled={isSubmitting}>{isSubmitting ? "Finalizando..." : "Finalizar Orden"}</Button>
                </div>
             </form>
        </Modal>
    );
