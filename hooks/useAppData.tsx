...
import { Role, User, Apartment, TaskType, Assignment, AssignmentStatus, CheckIn, ApartmentSize, WorkOrder, WorkOrderStatus, LogbookEntry, PostIt, PostItComment } from '../types';
...
const initialLogbookEntries: LogbookEntry[] = [];
...
const getInitialState = () => ({
  ...
  logbookEntries: initialLogbookEntries,
...
interface AppContextType {
  ...
  addLogbookEntry: (entry: Omit<LogbookEntry, 'id' | 'date' | 'createdBy'>) => void;
  updateLogbookEntry: (entry: LogbookEntry) => void;
  deleteLogbookEntry: (entryId: number) => void;
  addPostIt: (postItData: Omit<PostIt, 'id' | 'createdAt' | 'createdBy' | 'comments'>) => void;
  deletePostIt: (postItId: number) => void;
  addCommentToPostIt: (postItId: number, commentData: Omit<PostItComment, 'id' | 'createdAt' | 'createdBy'>) => void;
}
...
export const AppDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addLogbookEntry = useCallback((entry: Omit<LogbookEntry, 'id' | 'date' | 'createdBy'>) => {
    ...
    setData(prev => ({ ...prev, logbookEntries: [...prev.logbookEntries, newEntry] }));
  }, [currentUser]);

  const updateLogbookEntry = useCallback((updatedEntry: LogbookEntry) => {
    setData(prev => ({
      ...prev,
      logbookEntries: prev.logbookEntries.map(e => e.id === updatedEntry.id ? { ...updatedEntry, date: new Date().toISOString() } : e)
    }));
  }, []);

  const deleteLogbookEntry = useCallback((entryId: number) => {
    if (window.confirm('¿Está seguro que desea eliminar esta entrada de la bitácora?')) {
      setData(prev => ({
        ...prev,
        logbookEntries: prev.logbookEntries.filter(e => e.id !== entryId)
      }));
    }
  }, []);

  const addPostIt = useCallback((postItData: Omit<PostIt, 'id' | 'createdAt' | 'createdBy' | 'comments'>) => {
    if (!currentUser) return;
    ...
    setData(prev => ({ ...prev, postIts: [newPostIt, ...prev.postIts] }));
  }, [currentUser]);

  const deletePostIt = useCallback((postItId: number) => {
    if (window.confirm('¿Está seguro que desea eliminar esta nota?')) {
      setData(prev => ({ ...prev, postIts: prev.postIts.filter(p => p.id !== postItId) }));
    }
  }, []);

  const addCommentToPostIt = useCallback((postItId: number, commentData: Omit<PostItComment, 'id' | 'createdAt' | 'createdBy'>) => {
    if (!currentUser) return;
    const newComment: PostItComment = {
      ...commentData,
      id: Date.now(),
      createdAt: new Date().toISOString(),
      createdBy: currentUser.id,
    ...
    setData(prev => ({
      ...prev,
      postIts: prev.postIts.map(p =>
        p.id === postItId
          ? { ...p, comments: [...p.comments, newComment].sort((a,b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()) }
          : p
      ),
    }));
  }, [currentUser]);
